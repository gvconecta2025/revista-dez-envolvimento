...
// 1. INICIALIZAÇÃO DO EDITOR DE TEXTOS PREMIUM
let quill = null;
try {
...
} catch (error) { console.error("Erro ao carregar o Quill.js:", error); }

// 2. LÓGICA DE ABAS (3 ABAS ORIGINAIS)
...
async function carregarGerenciamento() {
    ...
    } catch (error) { manageList.innerHTML = "<p style='color:red; text-align:center;'>Erro ao carregar artigos.</p>"; }
}
...
manageList.addEventListener("click", async (e) => {
    ...
    if (action === "delete") {
        if (confirm("Tem certeza que deseja EXCLUIR este artigo definitivamente?")) {
            btn.textContent = "Apagando...";
            try {
                await deleteDoc(doc(db, "posts", id));
                carregarGerenciamento(); 
            } catch (error) { alert("Erro ao excluir: " + error.message); btn.textContent = "Excluir"; }
        }
    }
    if (action === "edit") { const post = memoriaPosts[id]; prepararEdicao(id, post); }
});
...
if(formPost) {
    formPost.addEventListener("submit", async (e) => {
        ...
        btnSubmitPost.disabled = true; btnSubmitPost.textContent = "Salvando...";

        try {
            if (editId) {
                await updateDoc(doc(db, "posts", editId), { titulo, categoria, conteudo: conteudoFormatado });
        ...
        finally { btnSubmitPost.disabled = false; btnSubmitPost.textContent = editId ? "Atualizar Artigo" : "Publicar Artigo"; }
    });
}

// 5. MOTOR DO DASHBOARD ANALÍTICO
...
async function carregarDashboard() {
    ...
    try {
        ...
        if (querySnapshot.empty) { dashList.innerHTML = "<p style='text-align:center;'>Nenhuma pergunta registrada.</p>"; return; }
    ...
    } catch (error) { dashList.innerHTML = `<p style='color:red;'>Erro ao ler Firebase: ${error.message}</p>`; }
}
